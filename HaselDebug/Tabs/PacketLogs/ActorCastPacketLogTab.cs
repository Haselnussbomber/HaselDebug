...
public unsafe partial class ActorCastPacketLogTab : PacketLogTab<ActorCastPacket>, IDisposable
{
    ...
    {
        _hook ??= _gameInteropProvider.HookFromAddress<PacketDispatcher.Delegates.HandleActorCastPacket>((nint)PacketDispatcher.MemberFunctionPointers.HandleActorCastPacket, HandleActorCastPacketDetour);
        ...
        using var table = ImRaii.Table("ActorCastPacketLogTable"u8, 2, ImGuiTableFlags.Borders | ImGuiTableFlags.ScrollY | ImGuiTableFlags.RowBg | ImGuiTableFlags.Resizable);
        if (!table) return;
