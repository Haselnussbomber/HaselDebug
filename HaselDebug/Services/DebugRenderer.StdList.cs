        ...
        using var node = DrawTreeNode(nodeOptions.WithSeStringTitle($"{elementCount} value{(elementCount != 1 ? "s" : "")}") with
        {
            DrawContextMenu = (nodeOptions, builder) =>
            {
                ...
                builder.Add(new ImGuiContextMenuEntry()
                {
                    ...
                    ClickCallback = () => _windowManager.Open(new PointerTypeWindow(_windowManager, _textService, _languageProvider, this, address, typeof(StdList<>).MakeGenericType(valueType), "0x" + address.ToString("X")))
                });
            }
        });
