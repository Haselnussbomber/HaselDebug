        ...
        using var node = DrawTreeNode(nodeOptions.WithSeStringTitle($"{elementCount} value{(elementCount != 1 ? "s" : "")}") with
        {
            DrawContextMenu = (nodeOptions, builder) =>
            {
                ...
                builder.Add(new ImGuiContextMenuEntry()
                {
                    ...
                    ClickCallback = () => WindowManager.Open(new PointerTypeWindow(WindowManager, TextService, LanguageProvider, this, address, typeof(StdList<>).MakeGenericType(valueType), "0x" + address.ToString("X")))
                });
            }
        });
